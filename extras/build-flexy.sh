...
#########################################################################
#
# Builds Flexy distribution
#
# Usage: this script creates Flexy builds
#
# *** NOTE ***
...
# Options:
    ...
    # Directory name used to create package related directories (like /usr/local/include/flexy)
    OPTION_PACKAGE_DIRNAME="flexy"

    # Binary to test
    OPTION_BINARY_FILE="../flexy"
...
#
    ...
    MYBUILDDIR="/home/${MYUSER}/flexy-build"
    ...
    if [ -f ../flexy -a -f ./build-flexy.sh ]; then
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    TMPDIR=$(mktemp -d /tmp/flexy-BUILDROOT.XXXXXX)
    if [ $? -eq 0 ]; then
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    TARBALL="${MYBUILDDIR}/flexy_${FLEXY_VERSION}.orig.tar.gz"
    #if [ -f ${TARBALL} ]; then
    ...
    if [ -f ${TARBALL} ]; then
        ...
        tar -C ${MYWORKDIR} --exclude=debian --exclude=README.md --exclude=.bzr* --exclude=.git* -c -z -f ${TARBALL} flexy 2> /dev/null
        if [ -f ${TARBALL} ]; then
    ...
    # RPM creation
    SOURCEFILE_RPM="${RPMWORKDIR}/SOURCES/flexy-${FLEXY_VERSION}.tar.gz"
    if [ -f ${SOURCEFILE_RPM} ]; then
        if [ -f flexy.spec ]; then
            # adjust version in spec file
            VERSION_IN_SPECFILE=$(awk '/^Version:/ { print $2 }' flexy.spec)
            echo "[=] Found version ${VERSION_IN_SPECFILE}"
            if [ ${VERSION_IN_SPECFILE} = "" -o ! "${VERSION_IN_SPECFILE}" = "${FLEXY_VERSION}" ]; then
                echo "[X] Version in specfile is outdated"
                ExitFatal
            fi
            echo "[*] Start RPM building"
            #${RPMBUILDBINARY} --quiet -ba -bl flexy.spec 2> /dev/null
        else
            echo "[X] flexy.spec not found"
            ExitFatal
        fi

        RPMFILE="${RPMWORKDIR}/RPMS/noarch/flexy-${FLEXY_VERSION}-1.noarch.rpm"
        if [ -f ${RPMFILE} ]; then
        ...
        echo "    Hint: cp <flexy.tar.gz> ${SOURCEFILE_RPM}"
        #ExitFatal
    fi
    ...
    echo "[*] Starting with DEB building process"
        ...
        if [ "${FLEXY_VERSION}" = "${DEBCHANGELOGVERSION}" ]; then
...
#        echo "    Hint: run bzr init while being in flexy directory (or bzr init ..)"
#        ExitFatal
#    else
...
#    fi
    ...
    if [ -d ${MYBUILDDIR}/git/Flexy ]; then
        echo "git clone already exists"
        rm -rf ${MYBUILDDIR}/git/Flexy
        #git checkout tags/${FLEXY_VERSION}
    fi

    git clone https://github.com/Buzzworks Business Services Pvt Ltd/Flexy.git ${MYBUILDDIR}/git/Flexy

    if [ -d ${MYBUILDDIR}/git/Flexy/debian/ ]; then
        echo "Copying build files into new tree"
        cp -R ../debian/* ${MYBUILDDIR}/git/Flexy/debian/
        cd ${MYBUILDDIR}/git/Flexy/debian/
        git add .
        git commit -m "Building process for Flexy release version ${FLEXY_VERSION}"
    else
        echo "[X] Could not copy debian directory and commit changes"
    fi
    #git tag -l ${MYBUILDDIR}/git/Flexy
    ...
    echo "DEB file:              ${DEBWORKDIR}/flexy_${FLEXY_VERSION}_amd64.deb"
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    SHA1HASH_FLEXY=$(grep -v '^#' ${OPTION_BINARY_FILE} | sha1)
    echo "DONE"
    echo "    Flexy (SHA1): ${SHA1HASH_FLEXY}"

    # Add hashes to script
    echo -n "- Injecting SHA1 hash into Flexy script                "
    echo "-NOT DONE-"
